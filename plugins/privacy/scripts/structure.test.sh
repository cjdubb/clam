#!/bin/bash
# Structural/contract tests for B03 privacy.
#
# Source of truth: the HTML-comment docblock labeled "Contract: B03
# privacy" in plugins/privacy/skills/setup/SKILL.md. This is a
...
#
# Covers plugins/privacy/.claude-plugin/plugin.json:
#   - valid JSON; has the required fields (name, description, version,
#     author); name is "privacy"; version is non-empty; description is
...
#     hardcoded copy)
#
# Covers plugins/privacy/skills/setup/SKILL.md:
...
#     non-empty
#   - skill name "setup" does not repeat/contain the plugin name "privacy"
#     (repo convention, matches plugins/attribution's and plugins/settings's
#     rule)
#   - no leftover NotImplemented marker anywhere in the raw file EXCEPT the
#     one deliberately-stub `<!-- NotImplemented: B03` line
...
#       * all five env var names: CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC,
#         DISABLE_TELEMETRY, DISABLE_ERROR_REPORTING,
#         DISABLE_FEEDBACK_COMMAND, CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY
#       * the value "1" for the env vars
#       * the `feedbackSurveyRate` top-level setting with value 0
...
# location).
#
# Run: bash plugins/privacy/scripts/structure.test.sh (exits non-zero on
# failure)
...
check "plugin.json .name is 'privacy'" "$pj_name" "privacy"
...
# ---------------------------------------------------------------------------
# 4. Skill name convention: "setup", not "privacy-setup"
# ---------------------------------------------------------------------------

check "SKILL.md skill name does not repeat/contain the plugin name 'privacy'" \
  "$(grep -qi 'privacy' <<<"$fm_name" && echo contains || echo ok)" "ok"

# ---------------------------------------------------------------------------
# 5. No leftover NotImplemented marker outside the deliberate stub line
# ---------------------------------------------------------------------------

stray_markers=$(grep -n 'NotImplemented' "$SKILL_MD" 2>/dev/null | grep -v 'NotImplemented: B03')
check "no NotImplemented markers outside the expected '<!-- NotImplemented: B03' stub line" \
...
check "instructions mention the CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC env var" \
  "$(grep -qF 'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC' <<<"$instructions" && echo yes || echo no)" "yes"
check "instructions mention the DISABLE_TELEMETRY env var" \
  "$(grep -qF 'DISABLE_TELEMETRY' <<<"$instructions" && echo yes || echo no)" "yes"
check "instructions mention the DISABLE_ERROR_REPORTING env var" \
  "$(grep -qF 'DISABLE_ERROR_REPORTING' <<<"$instructions" && echo yes || echo no)" "yes"
check "instructions mention the DISABLE_FEEDBACK_COMMAND env var" \
  "$(grep -qF 'DISABLE_FEEDBACK_COMMAND' <<<"$instructions" && echo yes || echo no)" "yes"
check "instructions mention the CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY env var" \
  "$(grep -qF 'CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY' <<<"$instructions" && echo yes || echo no)" "yes"

check "instructions mention the value \"1\" for the env vars" \
  "$(grep -qF '"1"' <<<"$instructions" && echo yes || echo no)" "yes"

check "instructions mention the feedbackSurveyRate top-level setting" \
  "$(grep -qF 'feedbackSurveyRate' <<<"$instructions" && echo yes || echo no)" "yes"
check "instructions mention feedbackSurveyRate's value of 0" \
  "$(grep -qE 'feedbackSurveyRate[^0-9]{0,20}\b0\b' <<<"$instructions" && echo yes || echo no)" "yes"

check "instructions mention the 'env' key/object in settings" \
  "$(grep -qw 'env' <<<"$instructions" && echo yes || echo no)" "yes"
