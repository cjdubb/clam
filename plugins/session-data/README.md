<!--
SCAFFOLD Contract: B05 session-data-readme (plan 002-readme-conformance)
...
Outputs:
  ...
  (/plugin marketplace add cjdubb/clam; /plugin install session-data@clam).
  Uninstalling opens with /plugin uninstall session-data@clam plus any cleanup.
Errors:
...
Edge cases / plugin-specific mapping:
  Sources: skills/, scripts/, .claude-plugin/plugin.json (plugin added in
  PR #92 to locate conversation/session data files). Derive purpose,
  commands, and workflows entirely from these sources; escalate if the
  purpose is not clear from them.
-->
