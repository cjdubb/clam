...
#     TROUBLESHOOTING.md, FOLLOWUPS.md, WORKGRAPH.md from .local/. Each file
#     is preceded by a "--- .local/<filename> ---" separator.
...
recovery_text=$(
    ...
    #
    # Contract: B05 — workgraph-lifecycle (recovery leg, plan 001-tracking-work-graph)
    # Behavior: WORKGRAPH.md joins the same fixed re-injection list, appended
    #   after FOLLOWUPS.md, so the work graph (nodes, Focus pointer) re-enters
    #   context after compaction (dump-if-present semantics and framing lines
    #   unchanged).
    # Invariants: list order otherwise preserved; the header comment naming
    #   the dumped files is updated to match.
    has_files=false
    for file in TODO.md PLAN.md IMPLEMENTATION-PLAN.md TROUBLESHOOTING.md FOLLOWUPS.md WORKGRAPH.md; do
