<!--
SCAFFOLD Contract: B03 privacy-readme (plan 002-readme-conformance)
...
Outputs:
  ...
  (/plugin marketplace add cjdubb/clam; /plugin install privacy@clam).
  Uninstalling opens with /plugin uninstall privacy@clam plus any cleanup.
Errors:
...
Edge cases / plugin-specific mapping:
  Only component: skills/setup/SKILL.md. Same shape as attribution: a
  /privacy:setup command (and "setup remove") writing settings key(s).
  Document the exact key(s) and values from the SKILL.md contract — do not
  guess them. Same inert-until-setup pattern: verify and state it.
-->
