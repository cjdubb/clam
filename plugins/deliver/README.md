<!--
SCAFFOLD Contract: B08 deliver-readme (plan 002-readme-conformance)
...
Outputs:
  ...
  (/plugin marketplace add cjdubb/clam; /plugin install deliver@clam).
  Uninstalling opens with /plugin uninstall deliver@clam plus any cleanup.
Errors:
...
Edge cases / plugin-specific mapping:
  Current H2s: Purpose, Companion plugins, Delivery lifecycle, Skills,
  Hook, Standing instructions. Purpose -> the H1 paragraph; Companion
  plugins -> Relationships; Delivery lifecycle -> What to expect and/or
  Common workflows as the content dictates; Skills, Hook, Standing
  instructions -> Commands (a standing instruction that describes ambient
  behavior belongs in What to expect; decide from content, escalate if
  unclear). Preserve existing contract comments verbatim.
-->
