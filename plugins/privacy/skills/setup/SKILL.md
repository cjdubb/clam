...
<!--
Contract: B05 setup-version-stamp — privacy (plan 001-update-flow-for-users)
Behavior (extension, to be added): after a successful install write (the
  verify step passes), record a setup stamp
  {plugin: "privacy", version, scope, target, at} in the stamp file per
  plugins/updates/docs/setup-stamps.md; on `/privacy:setup remove`, delete
