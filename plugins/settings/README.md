<!--
SCAFFOLD Contract: B04 settings-readme (plan 002-readme-conformance)
...
Outputs:
  ...
  (/plugin marketplace add cjdubb/clam; /plugin install settings@clam).
  Uninstalling opens with /plugin uninstall settings@clam plus any cleanup.
Errors:
...
Edge cases / plugin-specific mapping:
  Only component: skills/setup/SKILL.md. /settings:setup interactively
  writes session defaults: env keys (two hardcoded, one optional
  compact-window value) plus model, effortLevel, permissions.defaultMode;
  "setup remove" deletes exactly what was written. Document every key under
  the command per the SKILL.md contract. Explicit opt-in: installing changes
  nothing until setup runs — state this in What to expect.
-->
