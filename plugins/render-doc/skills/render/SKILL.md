...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/render.sh <doc.md> [--open|--serve] [--node=<id>]
```
...
- `--node=<id>` (with `--open` only) appends `#<id>` to the served URL, so a work-graph opens with that node's detail panel already docked beside the graph. Use it to land a checkpoint on the exact node under review.
...
| Schema-aware | Keyed on the H1: `Plan:` gets approach cards, edge-case treatment, changelog timeline; `Decision:` gets side-by-side option cards, a recommendation banner, status pills; `Design Questions:` gets, per `## DQ<n>:` section, side-by-side option cards with pros/cons and an inline recommendation banner (a non-conforming DQ falls back to baseline for that section only); `# Work Graph` gets a tree of node cards nested by Parent edges, a dependency badge per Deps entry, a three-way status pill (open, done, or dropped) per node, and a focus banner for the Focus node, with per-node fallback to baseline for a non-conforming node — a topbar "Graph" toggle switches that card tree to a node-and-edge view rendered with cytoscape and a dagre layout (status-colored nodes, Parent decomposition drawn as nested compound boxes with dashed dep arrows as the only edges, a Focus highlight, and a tap on a node splitting the screen — graph docked left, a full-height right panel with that node's complete detail: id, title, a status pill with reason, goal, Parent and Deps as clickable chips that repaint the panel for that node, Notes with its links live, and (served pages only) the first Notes-linked markdown section fetched and rendered inline below the fields — a "Go to card" control in the panel drops back to the card and scrolls to it, and a panel that cannot open degrades to sending the node back to its card), with the graph view the default on load and the card tree one toggle away. A topbar toggle switches to generic rendering; unrecognized H1s get baseline only |
| Cross-document links (served pages only) | Prose references to repo markdown documents — bare or backticked `.md` path tokens, absolute or relative to the document's own directory — link to that file's server view; author-written links and fenced code blocks are untouched, no existence check is attempted (a missing target clicks through to the server's JSON error), a failure leaves the prose plain and the page rendered, and `file://` pages are unaffected |
...
When the artifact under review is linked from a work-graph node, the checkpoint open targets the graph, not the bare artifact: `render.sh <workgraph.md> --open --node=<the gate node's id>`. The node panel hydrates the linked artifact inline, so the plan or decision is read in place while the one persistent surface — the graph — stays primary across the whole run. The bare-artifact `--open` remains the fallback when no graph exists or no node links the artifact.
