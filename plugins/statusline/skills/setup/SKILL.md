...
3. **Merge, don't overwrite.** Set the three keys in one pass over the
   existing file, preserving every other setting in it:
...
`statusLine`, `subagentStatusLine` and `refreshInterval` — preserving all
other settings, for example with
`jq 'del(.statusLine, .subagentStatusLine, .refreshInterval)'` over the
existing file, written through a temp file and `mv` as above. If the user
