...
#
# Covers plugin.json:
#   - valid JSON; name "voice"; version "0.3.0" exactly; non-empty
...
# Retargeted 0.2.0 -> 0.3.0 by the SessionStart-hook -> output-styles
# conversion. The pin tracks the CURRENT version, so every legitimate bump
# retargets it.
pj_version=$(jq -r '.version // empty' "$PLUGIN_JSON" 2>/dev/null)
check "plugin.json .version is exactly '0.3.0'" "$pj_version" "0.3.0"
