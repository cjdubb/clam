...
check "plugin.json version is exactly 0.17.0" "$b07_plugin_version" "0.17.0"

EXPECTED_B07_DESCRIPTION='Tracking-document workflow: .local/TODO.md as session state of record, 13-state lifecycle with Stop-hook enforcement, a built-in task-tools deny, absorbed stall-recovery (capture hook + /make-progress skill), resume-after-/clear via SessionStart injection, a work graph (.local/WORKGRAPH.md) for recursive problem decomposition, and reporting-only situation reports (/sitrep and /pr-status).'
...
assert_contains_re_i "root README.md: tracking row's version cell is v0.17.0" "$tracking_row" \
    '✅ *v0\.17\.0'
