#!/bin/bash
# Structural/contract tests for B01 attribution.
#
# Source of truth: the HTML-comment docblock labeled "Contract: B01
# attribution" in plugins/attribution/skills/setup/SKILL.md. This is a
...
#
# Covers plugins/attribution/.claude-plugin/plugin.json:
#   - valid JSON; has the required fields (name, description, version,
#     author); name is "attribution"; version is non-empty; description is
...
#     hardcoded copy)
#
# Covers plugins/attribution/skills/setup/SKILL.md:
...
#     non-empty
#   - skill name "setup" does not repeat/contain the plugin name
#     "attribution" (repo convention, matches plugins/worktrees's rule)
#   - no leftover NotImplemented marker anywhere in the raw file EXCEPT the
#     one deliberately-stub `<!-- NotImplemented: B01` line
...
#       * the `attribution` setting key
#       * both the `commit` and `pr` subkeys
...
# location).
#
# Run: bash plugins/attribution/scripts/structure.test.sh (exits non-zero on
# failure)
...
check "plugins/attribution/ exists" \
...
check "plugin.json .name is 'attribution'" "$pj_name" "attribution"
...
# ---------------------------------------------------------------------------
# 4. Skill name convention: "setup", not "attribution-setup"
# ---------------------------------------------------------------------------

check "SKILL.md skill name does not repeat/contain the plugin name 'attribution'" \
  "$(grep -qi 'attribution' <<<"$fm_name" && echo contains || echo ok)" "ok"

# ---------------------------------------------------------------------------
# 5. No leftover NotImplemented marker outside the deliberate stub line
# ---------------------------------------------------------------------------

stray_markers=$(grep -n 'NotImplemented' "$SKILL_MD" 2>/dev/null | grep -v 'NotImplemented: B01')
check "no NotImplemented markers outside the expected '<!-- NotImplemented: B01' stub line" \
...
check "instructions mention the 'attribution' setting key" \
  "$(grep -qi 'attribution' <<<"$instructions" && echo yes || echo no)" "yes"

check "instructions mention the 'commit' subkey" \
  "$(grep -qiw 'commit' <<<"$instructions" && echo yes || echo no)" "yes"
check "instructions mention the 'pr' subkey" \
  "$(grep -qiw 'pr' <<<"$instructions" && echo yes || echo no)" "yes"
